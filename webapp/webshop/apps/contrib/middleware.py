...
class CampaignTracker(object):
    def process_request(self, request):
        if request.GET.get('cid'):
            ...
            cidlist.append({'cid': request.GET.get('cid'), 'timestamp': datetime.now(),
...
def random_promo_processor(request):
    ...
    if not total_article_count:
        count = ArticleOption.objects.filter(
            article__article_family__line__shop__allowed_shops__icontains=settings.SHOP_NAME).count()
        count -= count % 10
...
class ErrorEmailingMiddleware:
    def process_exception(self, request, exception):
        if request.user.is_authenticated():
            user = (
            request.user.id, request.user.username, ['%s' % g for g in request.user.groups.all()], request.user.email)
        else:
            user = 'anonymous'

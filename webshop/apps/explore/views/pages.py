...
def render_with_locale(request, template, context):
    context.update({'short_locale':request.LANGUAGE_CODE[:2].lower()})
    short_locale = request.LANGUAGE_CODE[:2].lower()
    try:
        return render_to_response('pages/%s_%s.html' % (template, short_locale), context, context_instance=RequestContext(request))
    except TemplateDoesNotExist:
        return render_to_response('pages/%s_en.html' % template, context, context_instance=RequestContext(request))

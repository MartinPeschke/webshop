...
def check_image(af, subdir, shop_ref):
    if af.logopath:
        if os.path.exists(os.path.join(settings.MEDIA_ROOT, 'files', shop_ref, subdir.replace('|', os.sep), af.logopath + '.jpg')):
            return mark_safe('/'.join((settings.MEDIA_URL, 'files', shop_ref, subdir.replace('|', '/'), af.logopath + '.jpg')))
    return mark_safe('/'.join((settings.MEDIA_URL, 'files', 'none_%s.png' % subdir.split('/')[-1])))
...
@register.filter
def create_ao_imgtag(ao, shop_ref=None):
    if ao.ref == '-':
    	return '-'
    if os.path.exists(os.path.join(settings.MEDIA_ROOT, 'files', shop_ref, 'options', ao.imgName)):
        return mark_safe('<div class="optionLink oldeselected" title="{ao_ref}"><img alt="{ao_ref}" class="small" src="/media/files/{shop_ref}/options/{img}"/></div>'.format(ao_ref = ao.ref, shop_ref = shop_ref, img = ao.imgName))
    else:
...
def create_ao_images(article_options, shop_ref):
    ...
    imgs = filter(lambda x: x!='' and x!='-', [create_ao_imgtag(ao, shop_ref) for ao in aos])
    if imgs:
        imgs = list(set(imgs))
        imgs.sort()
#===============================================================================

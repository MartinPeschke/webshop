...
def add_to_cart(request):
	...
	return {'added_id_qty':added, 
			'cart_html':render_to_string('explore/cart_items.html', locals(), context_instance=RequestContext(request))}
...
def update_cart(request):
	...
	return render_to_response('explore/cart_items.html', locals())
...
def shopping_cart(request):
	...
	return render_to_response('user/order/shopping_cart.html', locals(), context_instance=RequestContext(request))
...
def cashier(request):
	if(not request.user.is_authenticated() or not request.session.get('cart',None)):
		return HttpResponseRedirect('/user/login/?forward_URL=/user/shopping_cart/cashier/')

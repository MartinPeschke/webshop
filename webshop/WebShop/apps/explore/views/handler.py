...
def line(request, shop_ref, line_ref = None):
    ...
    line_html = mark_safe(render_to_string('explore/line_page.html', context,  context_instance=rctxt))
    #cache.set('line_page_%s_%s_%s_%s' % (line.id, request.GET.get('t', None), role, lang), line_html, settings.CACHE_TIMEOUT)
    
    context.update(locals())
    return render_to_response('explore/line.html', context, context_instance=rctxt)
